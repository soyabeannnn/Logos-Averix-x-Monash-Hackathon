...
describe("filtersToQuery", () => {
  it("omits empty values and returns an empty string when nothing is set", () => {
    ...
    expect(filtersToQuery({ status: "MISMATCH", category: "", q: "", archived: "" })).toBe("?status=MISMATCH");
  });
...
describe("filtersFromParams", () => {
  it("reads known keys and defaults the rest to empty strings", () => {
    ...
    expect(filtersFromParams(params)).toEqual({ category: "", status: "OK", q: "hello", archived: "" });
  });

  it("round-trips through the query string", () => {
    const filters = { category: "BL_COMPARISON", status: "NEEDS_REVIEW", q: "x y", archived: "1" };
